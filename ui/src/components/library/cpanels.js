...
function CPanels(props) {
  ...
  const handleItemModalSave = () => {
    ...
    axios.post("/api/plist", plist).then(() => {
      updateDisplay();
    });
  ...
  const handleModalSave = () => {
    ...
    //return;
    if (creationMode)
      axios.post("/api/cpanels", element).then(() => {
        setCPanels([
          ...cpanels,
          {
            data: element,
      ...
      axios.put("/api/cpanels", element).then((res) => {
        setCPanels(
          cpanels.map((item) => {
  ...
  const getCPanels = () => {
    axios.get("/api/cpanels").then((res) => {
      setCPanels(res.data);
    });
  };
  ...
  const dropCPanItem = (id) => {
    ...
    window.confirm("Are you sure you want to delete this simple panel?")
      ? axios.delete(`/api/plist/${id}`).then((res) => {
          setpList(
            pList.filter((item) => {
              return item.id !== id;
            })
  ...
  const getPanels = () => {
    axios.get("/api/panels").then((res) => {
      setPanels(res.data); console.log(res.data)
    });
  };

  const getpList = () => {
    axios.get("/api/plist").then((res) => {
      setpList(res.data);
    });
  };
  ...
  const getVisualizations = () => {
    axios.get("/api/visualizations").then((res) => {
