...
function Frames(props) {
  ...
  const handleItemModalSave = () => {
    ...
    axios.post("/api/flist", flist).then(() => {
      updateDisplay();
    });
  ...
  const handleModalSave = () => {
    ...
    //return;
    if (creationMode)
      axios.post("/api/frames", element).then(() => {
        setFrames([
          ...frames,
          {
            data: element,
      ...
      axios.put("/api/frames", element).then((res) => {
        setFrames(
          frames.map((item) => {
  ...
  const deleteFrame = (id) => {
    window.confirm("Are you sure you want to delete this type of screen?")
      ? axios.delete(`/api/frames/${id}`).then((res) => {
          setFrames(
            frames.filter((item) => {
              return item.id !== id;
            })
  ...
  const dropCPanItem = (id) => {
    ...
    window.confirm("Are you sure you want to delete this panel?")
      ? axios.delete(`/api/flist/{id}`).then((res) => {
          setfList(
            fList.filter((item) => {
              return item.id !== id;
            })
