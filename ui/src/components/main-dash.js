...
const MainDash = (props) => {
  ...
  const getWSs = () => {
    ...
    console.log("HEREEEEEEEEEEE: "+ process.env)
    axios.get("/api/wss").then((res) => {
      setWSs(res.data);
  ...
  const loadGrafana = () => {
    window.location.href = "http://" + window.location.hostname + ":3030"
  };
  ...
  const createNewWorkSpace = () => {
    ...
    if (found.length > 0)
     ...
     //   description:'Description of the WS'
      ...
      axios.post("/api/wss", element).then(() => {
        getWSs();
        setOpen(false);
      setNewWSName("");
      });
